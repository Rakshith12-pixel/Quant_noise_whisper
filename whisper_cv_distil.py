...
def train(args, accelerator):
    ...
    if args.adaptive_distill:
        t_losses=[]
        for batch in train_dataloader:
            with torch.no_grad():
                t_outputs = teacher(**batch)
                # teacher losses
                t_losses.append(t_outputs.loss.item())
        t = mean(t_losses)

    if args.save_losses:
        losses_path = f"./{args.data_lang}_losses.npy"
        np.save(losses_path,t_losses)
        ...
        for batch in train_dataloader:
            with accelerator.accumulate(model):
                ...
                if args.adaptive_distill:
                    ...
                    K-= ((K_MAX - K_MIN)/args.train_steps)
                    d = torch.exp(-K * ( s_loss - t))
                    alpha = torch.exp(-1/math.sqrt(d))
                
                    args.alpha_distil = alpha , args.alpha_ce = 1 - alpha 
              
                    loss = args.alpha_distil * d_loss + args.alpha_ce * s_loss

                else 

                    loss = 0.5 * d_loss + 0.5 * s_loss
