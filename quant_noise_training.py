...
from transformers import WhisperTokenizer
import modelutils
from modelutils import get_layers
...
#    tokenizer = WhisperTokenizer.from_pretrained(args.model_name_or_path, language=args.model_lang, task=args.task)
   ...
   # processor = WhisperProcessor.from_pretrained(args.model_name_or_path, language=args.model_lang, task=args.task)
    ...
    model = WhisperForConditionalGeneration.from_pretrained("/home/ujan/distillation-asr/whisper-models/pretrained/openai_whisper-small")
    print("model is in {}".format(model.device))
#    print(model.parameters())
#    for name, param in model.named_parameters():
#     if 'weight' in name and 'LayerNorm' not in name:
#      setattr(model,name,quant_noise(param,p=0.1,block_size=8))
...
#    common_voice["test"] = load_dataset(args.data_dir, args.data_lang, split="test")
    common_voice["train"] = load_from_disk("/media/ujan/MHST/common_voice_train")
    common_voice["test"] = load_from_disk("/media/ujan/MHST/common_voice_test")
...
#    print("MODULE IS {}".format(module)) #works
      if p <= 0 :
          return module
    
#      print("module is {}".format(module))
#    print("module is {}".format(module)) # works
      if "Conv1d" in str(module.__class__):
          return module
#    print("list(module.param)[0] is {}".format(list(module.parameters())[0]))
    # supported modules
#    assert isinstance(module, nn.Linear)
    #print("module is {}".format(module)) #doesnt work
    #assert isinstance(module, (nn.Linear, nn.Embedding, nn.Conv2d))

      is_conv = False #module.layer.weight.ndim == 4
    # 2D matrix
...
#      print("module is {}".format(module))
      if not is_conv:
          assert (
              parameters_to_vector(list(module.parameters())[0]).size(-1) % block_size == 0
          ...
          if mod.training:
              if not is_conv:
                # gather weight and sizes
                  weight = parameters_to_vector(list(mod.parameters())) #weight.shape = 241734912 the weight matrix sizeof whisper small
                  print("weight is {}".format(weight.shape))
                  print("size is {}".format(sys.getsizeof(weight)))
                  in_features = weight.size(-1)
                  out_features = weight.size(0) 
                  print("in features is {}".format(in_features))
        
                  print("out features is {}".format(out_features))
                ...
                # gather weight and sizes
                  weight = parameters_to_vector(list(mod.parameters()))
            ...
            #print("mod.weight.data is {}".format(mod.weight.data))

      module.register_forward_pre_hook(_forward_pre_hook)
    model.apply(quant_noise)
    print("applied quant noise")
