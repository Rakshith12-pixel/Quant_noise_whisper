...
from transformers import WhisperConfig, WhisperModel, WhisperForConditionalGeneration
...
# Initializing a model (with random weights) from the tiny style configuration
           ...
           # model = WhisperForConditionalGeneration(configuration)
            model = WhisperForConditionalGeneration.from_pretrained("/home/ujan/distillation-asr/whisper-models/finetuned/whisper-small/hindi/checkpoint") #whisper hindi checkpoint
            #model = WhisperForConditionalGeneration.from_pretrained("/home/ujan/distillation-asr/whisper-models/finetuned/whisper-small/chinese/checkpoint")
    model.seqlen = 2048

    print("Model loaded sucessfully ...")
    print("model size is {}".format(sys.getsizeof(model)))
    return model
...
def get_layers(model):
    if model.config.model_type == "llama":
        ...
        return model.model.encoder.layers+model.model.decoder.layers
    else:
        raise ValueError(MODEL_ERROR_MSG.format(model.config.model_type))
